<?php
	/*
		Plugin Name: Inline Posts updated by Anna
	...
	*/
	...
	{		
		...
		for ($i = 0; $i < $numMatches; $i++)
		{
			...
			$tags = the_tags($postId);
			$posttags = get_the_tags();

			
			$subtitle = '<span class="subtitle">'.get_post_meta($post->ID, '_visual-subtitle', true).'</span>';
			...
			$postTitleText = "<$titleTag>$linkToPost$subtitle$postTitle</a>$anchorTag</$titleTag>";
			//$postBodyText = format_to_post($post->post_content).'</p>';
			
			/* If you want to post an excerpt instead, with link to full post, comment out above postBodyText and comment in the following three lines */
			 $linkToPostForMore = get_permalink($postId);
			 $linkToMore = "<a href=\"$linkToPostForMore\">Continue reading...</a>";
			 $postBodyText = '<p>'.format_to_post($post->post_excerpt).' ';
			...
			$text = $postTitleText.'<small>'.$lastModifiedText.' '.$commentsText.' '.$topLink.' '.$editLink."</small>".$postBodyText.$linkToMore.$tags;
